import type { TradeData } from "../tradePages";

export const TEGELZETTERS_DATA: TradeData = {
  slug: "tegelzetters",
  singular: "tegelzetter",
  plural: "tegelzetters",
  bedrijf: "tegelzetbedrijf",
  metaTitle: "Website & Marketing voor Tegelzetters | Meer Klanten — Aannemer Systeem",
  metaDescription: "Speciaal voor tegelzetters: een professionele website, automatische reviews en lokale SEO. Wij helpen tegelzetbedrijven meer klanten te krijgen — zonder gedoe en zonder te concurreren op prijs.",
  hero: {
    badge: "Speciaal voor Tegelzetters",
    headline: "Meer Klanten voor Jouw Tegelzetbedrijf — Zonder Marketing Gedoe",
    subheadline: "Wij bouwen het complete online systeem zodat huiseigenaren jou vinden, vertrouwen en bellen. Jij legt de tegels. Wij regelen de rest.",
    stats: [
      { value: "87%", label: "van klanten zoekt online naar een tegelzetter" },
      { value: "3x", label: "meer aanvragen met een sterke Google positie" },
      { value: "7", label: "dagen en je nieuwe website staat live" },
    ],
  },
  problems: {
    sectionTitle: "Herken je dit als tegelzetter?",
    items: [
      { title: "Afhankelijk van mond-tot-mond en aannemers", description: "Je krijgt werk via vaste aannemers of via via, maar zodra één opdrachtgever wegvalt, merk je direct dat je eigen stroom aan klanten ontbreekt. Zonder online zichtbaarheid ben je overgeleverd aan andermans agenda." },
      { title: "Concurrentie op prijs via platforms", description: "Op Werkspot en vergelijkbare platformen sta je naast tientallen tegelzetters die elkaar overbieden op de laagste prijs. Klanten vergelijken alleen op kosten en zien niet wat jouw vakmanschap waard is." },
      { title: "Geen visueel bewijs van je vakmanschap", description: "Tegelwerk is een visueel vak — een strak betegelde badkamer of een perfect gelegde vloer spreekt boekdelen. Maar zonder professionele foto's en een sterke online presentatie ziet niemand het verschil tussen jou en de concurrent." },
      { title: "Leads die niet converteren", description: "Mensen bellen voor een 'prijsje' zonder te weten wat goed tegelwerk kost. Je besteedt uren aan offertes voor klanten die uiteindelijk kiezen voor de goedkoopste optie. Zonder vooraf opgebouwd vertrouwen trek je de verkeerde klanten aan." },
    ],
  },
  solution: {
    sectionTitle: "Hoe wij tegelzetters laten groeien",
    intro: "Ons systeem is specifiek ingericht voor tegelzetbedrijven. Geen generieke template — maar een aanpak die jouw tegelwerk laat zien, in jouw regio, aan de juiste klant.",
    items: [
      { title: "Een website die jouw tegelwerk laat spreken", description: "Een professionele website met aparte pagina's per specialisme: badkamertegels, keukentegels, vloertegels, wandtegels, mozaïek en natuursteen. Elke pagina bevat voor/na-foto's en is geoptimaliseerd op lokale zoekopdrachten zoals 'tegelzetter [jouw stad]' of 'badkamer betegelen'.", keywords: ["badkamertegels", "vloertegels", "wandtegels", "mozaïek", "natuursteen", "tegelzetter"] },
      { title: "Lokale SEO die jou bovenaan Google zet", description: "Wij optimaliseren je Google Mijn Bedrijf profiel volledig: categorieën, diensten, werkgebied, foto's van afgerond tegelwerk en een gekoppelde website. Daarnaast schrijven wij pagina's die scoren op zoekopdrachten als 'tegelzetter in de buurt' of 'badkamer laten betegelen'.", keywords: ["google mijn bedrijf", "lokale seo", "tegelzetter in de buurt", "vindbaarheid"] },
      { title: "Automatische reviews na elke opdracht", description: "Na elk project sturen wij automatisch een reviewverzoek naar je klant. Tevreden? Direct naar Google. Niet tevreden? Eerst naar jou, zodat je het kunt oplossen. Zo bouw je een reviewprofiel op dat bewijst dat jouw tegelwerk van topkwaliteit is.", keywords: ["google reviews", "online reputatie", "klanttevredenheid", "5 sterren"] },
      { title: "Gemiste oproep? Direct een automatisch bericht terug", description: "Bezig met tegels leggen en kun je niet opnemen? Wij sturen automatisch een SMS: 'Bedankt voor je telefoontje, ik neem zo snel mogelijk contact op.' Klanten haken niet af, leads gaan niet verloren.", keywords: ["lead opvolging", "gemiste oproepen", "sms automatisering", "klantbehoud"] },
    ],
  },
  industryContent: {
    sectionTitle: "Marketing voor tegelzetters: wat werkt in 2026?",
    paragraphs: [
      "De vraag naar tegelzetters in Nederland blijft sterk. Badkamerrenovaties, keukenverbouwingen en vloervernieuwingen zijn populaire projecten onder huiseigenaren. Maar de concurrentie groeit mee: steeds meer tegelzetters melden zich aan op vergelijkingsplatforms, en klanten vergelijken online voordat ze een vakman kiezen. Wie niet zichtbaar is op Google, mist het grootste deel van de markt.",
      "Platformen als Werkspot en Homedeal beloven directe leads, maar de praktijk is anders. Je betaalt €20 tot €60 per lead, concurreert met vijf tot tien andere tegelzetters op dezelfde klus, en hebt geen controle over je presentatie. Met een eigen website en sterk Google-profiel trek je klanten aan die al overtuigd zijn van jouw kwaliteit voordat ze contact opnemen.",
      "Tegelwerk is bij uitstek een visueel vak. Een professionele portfolio met voor/na-foto's van badkamers, keukens en vloeren doet meer dan welke tekst dan ook. In 2026 verwachten klanten dat ze jouw werk online kunnen bekijken, reviews kunnen lezen en direct een offerte kunnen aanvragen. Een website zonder beelden is een gemiste kans.",
      "De sleutel tot online succes als tegelzetter is specialisatie in je communicatie. Maak aparte pagina's voor badkamertegels, vloertegels, wandtegels en natuursteen. Elke pagina trekt een ander type zoeker aan en scoort op specifieke zoektermen. Combineer dit met een stroom van verse Google reviews en je bouwt een online reputatie die klanten overtuigt nog voordat ze bellen.",
    ],
  },
  comparison: {
    sectionTitle: "Waarom niet gewoon Werkspot of een duur bureau?",
    rows: [
      { aspect: "Kosten per lead", werkspot: "€20–€60 per lead, ongeacht kwaliteit", bureau: "€750–€2.500/maand + opstartkosten", groeisysteem: "€279/maand vast — onbeperkte leads" },
      { aspect: "Eigenaarschap website", werkspot: "Geen eigen website", bureau: "Vaak vastgezet in contract", groeisysteem: "Jouw website, altijd in jouw beheer" },
      { aspect: "Reviews automatisering", werkspot: "Reviews alleen op het platform", bureau: "Zelden inbegrepen", groeisysteem: "Volledig automatisch na elk project" },
      { aspect: "Lokale SEO", werkspot: "Je concurreert met tientallen tegelzetters op het platform", bureau: "Vaak apart geprijsd", groeisysteem: "Inbegrepen en specifiek ingericht voor tegelzetters" },
      { aspect: "Opzettijd", werkspot: "Direct maar nul controle", bureau: "6–14 weken", groeisysteem: "Live binnen 7 dagen" },
    ],
  },
  results: {
    sectionTitle: "Resultaten voor tegelzetters",
    items: [
      { metric: "Van 6 naar 54 Google reviews", description: "In 4 maanden, volledig automatisch na elk tegelproject" },
      { metric: "Pagina 1 in Google Maps", description: "Voor 'tegelzetter [stad]' binnen 3 maanden" },
      { metric: "15 nieuwe aanvragen per maand", description: "Via de eigen website, zonder te betalen voor leads" },
    ],
  },
  faqs: [
    { question: "Hoeveel kost een website voor een tegelzetbedrijf?", answer: "Bij Aannemer Systeem betaal je €279 per maand voor een complete website inclusief hosting, onderhoud, lokale SEO en marketing automatiseringen. Er zijn geen opstartkosten. Je website staat binnen 7 dagen live." },
    { question: "Hoe zorg ik dat mijn tegelzetbedrijf gevonden wordt op Google?", answer: "De drie pijlers: (1) Een professionele website met aparte pagina's per specialisme (badkamertegels, vloertegels, wandtegels). (2) Een volledig Google Mijn Bedrijf-profiel met foto's van afgerond tegelwerk. (3) Een constante stroom van Google reviews boven de 4,5 sterren. Ons systeem automatiseert al deze drie stappen." },
    { question: "Hoe onderscheid ik mij van goedkopere tegelzetters?", answer: "Voor/na-foto's bewijzen je vakmanschap beter dan woorden. Reviews op Google laten zien dat eerdere klanten tevreden zijn. Een professionele website geeft vertrouwen en toont je specialisatie. Klanten die kwaliteit willen, zoeken dit bewijs online." },
    { question: "Wat is het verschil met Werkspot?", answer: "Op Werkspot sta je naast tien andere tegelzetters en vergelijkt de klant op prijs. Met Aannemer Systeem bouw je een eigen online aanwezigheid: klanten vinden jou via Google, bekijken je portfolio, lezen je reviews en bellen jou rechtstreeks." },
    { question: "Kan ik mijn bestaande portfolio meenemen?", answer: "Absoluut. Wij verwerken je bestaande foto's van badkamers, keukens en vloerprojecten in je nieuwe website. Heb je nog geen professionele foto's? Wij adviseren je hoe je met je telefoon al sterke voor/na-beelden maakt." },
    { question: "Hoe snel zie ik resultaat?", answer: "Google Mijn Bedrijf reageert snel op optimalisaties — verbeteringen zijn vaak al binnen weken zichtbaar. Organische zoekresultaten bouwen over 2 tot 4 maanden op. Tegelzetters bereiken gemiddeld pagina 1 binnen 3 maanden." },
  ],
  relatedTrades: [
    { label: "Badkamerrenovatie", slug: "badkamerrenovatie" },
    { label: "Loodgieters", slug: "loodgieters" },
    { label: "Stukadoors", slug: "stukadoors" },
    { label: "Keukenmonteurs", slug: "keukenmonteurs" },
  ],
  cta: {
    headline: "Klaar om meer tegelprojecten binnen te halen?",
    subheadline: "Boek een gratis gesprek en we laten zien hoe het systeem werkt voor tegelzetters in jouw regio.",
    buttonText: "Boek een Demo",
    buttonUrl: "/demo",
  },
};
